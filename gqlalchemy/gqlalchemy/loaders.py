...
class Translator:
    ...
    def _load_nodes(self) -> None:
        for collection_mapping in self._collection_mappings:
            for row in self._data_source.load_data(collection_name=collection_mapping.collection_name):
                self._save_row_as_node(
                    label=self._get_new_name_for_collection(collection_mapping.collection_name), row=row
                )
        ...
        for cross_relationship_mapping in self._cross_relationship_mappings:
            ...
            for row in self._data_source.load_data(collection_name=collection_name, is_cross_table=True):
                ...
                self._save_row_as_relationship(
                    relations=[mapping1.reference_table, mapping2.reference_table],
                    on_properties=[mapping1.reference_key, mapping2.reference_key],
    ...
    def _create_triggers(self) -> None:
        for collection_mapping in self._collection_mappings:
            label1 = self._get_new_name_for_collection(collection_mapping.collection_name)
            for nx1_mapping in collection_mapping.mapping:
                property1 = self._get_new_name_for_column_in_collection(
                    collection_mapping.collection_name, nx1_mapping.mapping.foreign_key
                )
                label2 = self._get_new_name_for_collection(nx1_mapping.mapping.reference_table)
                property2 = self._get_new_name_for_column_in_collection(
                    collection_mapping.collection_name, nx1_mapping.mapping.reference_key
                )
    ...
    def _create_indices(self) -> None:
        for collection_mapping in self._collection_mappings:
            collection_name = self._get_new_name_for_collection(collection_mapping.collection_name)
            for index in collection_mapping.indices:
                new_index = self._get_new_name_for_column_in_collection(collection_mapping.collection_name, index)
    ...
    def _create_trigger_cypher_query(
        ...
        return (
            Unwind(list_expression="createdVertices", variable="a")
            .with_(results={"a": ""})
    ...
    def _save_row_as_node(self, label: str, row: Dict[str, Any], index=None):
        if index:
            self._memgraph.create_index(MemgraphIndex(label, index))

        query_builder = QueryBuilder(connection=self._memgraph)
        list(
            query_builder.create()
            .node(
                labels=self._get_new_name_for_collection(label),
                **{self._get_new_name_for_column_in_collection(label, k): v for k, v in row.items()},
            )
            .execute()
        )
    ...
    def _save_row_as_relationship(
        ...
        label_from, label_to = self._get_new_name_for_collection(relations[0]), self._get_new_name_for_collection(
            relations[1]
        )
        property_from, property_to = on_properties[0], on_properties[1]

        query_builder = QueryBuilder(connection=self._memgraph)
        list(
            query_builder.match()
            .node(labels=label_from, variable=from_node, **{property_from: row[property_from]})
            .match()
            .node(labels=label_to, variable=to_node, **{property_to: row[property_to]})
            .create()
            .node(variable=from_node)
            .to(relation_label)
            .node(variable=to_node)
    ...
    def _get_new_name_for_collection(self, original_name: str) -> str:
    ...
    def _get_new_name_for_column_in_collection(self, collection_name: str, original_column_name: str):
    ...
    def _load_nx1_relations_and_indices(
        self, nx1_relations: Dict[str, List[str]], indices: Dict[str, List[str]]
    ) -> None:
        for table_name, relations in nx1_relations.items():
            self._collection_mappings.append(
                CollectionMapping(
    ...
    def _load_mxn_relations(self, mxn_relations: Dict):
        for table_name, relations in mxn_relations.items():
            self._cross_relationship_mappings.append(
                CollectionMapping(
                    collection_name=table_name,
                    mapping=MxNMapping(
